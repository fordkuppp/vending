...
def test_category_api(app, models):
    post_response = app.test_client().post('/api/category', json={
        "name": "A"
    })
    assert post_response.status_code == 200
    with app.app_context():
        new_category_id = post_response.get_json()['id']
        category = models.db.session.query(models.Category).filter(models.Category.id == new_category_id).one()
    ...
    with app.app_context():
        with pytest.raises(NoResultFound):
            models.db.session.query(models.Category).filter(models.Category.id == category.id).one()
