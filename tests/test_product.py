...
def test_product_api(app, models):
    post_response = app.test_client().post('/api/product', json={
        "machine_id": "1",
        "name": "A",
        "quantity": "99",
        "price": "88"
    ...
    with app.app_context():
        ...
        product = models.db.session.query(models.Product).filter(models.Product.id == new_product_id).one()
    ...
    with app.app_context():
        with pytest.raises(NoResultFound):
            models.db.session.query(models.Product).filter(models.Product.id == product.id).one()
