...
def test_machine_api(app, models):
    post_response = app.test_client().post('/api/machine', json={
        "building_num": "99",
        "floor": "88"
    })
    ...
    with app.app_context():
        ...
        machine = models.db.session.query(models.Machine).filter(models.Machine.id == new_machine_id).one()
    ...
    with app.app_context():
        with pytest.raises(NoResultFound):
            models.db.session.query(models.Machine).filter(models.Machine.id == machine.id).one()
